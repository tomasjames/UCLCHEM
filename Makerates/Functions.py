...
def read_reaction_file(fileName, speciesList, ftype):
	...
	if ftype == 'KIDA':  # if it is a umist database file
		f = open(fileName, 'rb')
		reader = csv.reader(f, delimiter=',', quotechar='|')
		for row in reader:
			react_data = row[0:2]+['']+row[3:7]+row[8:11]+row[15:17]
			# if all the reaction elements belong to the keeplist
			if all(x in keepList for x in [row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7]]):
				#umist file doesn't have third reactant so add space and has a note for how reactions there are so remove that
				reactions.append(Reaction(row[0:2]+['']+row[3:7]+row[8:11]+row[15:17]))
